...
const raisedToTheThird = (arr) => { // Declare a function named raisedToTheThird which takes an array as input
  const result = []; // Initialize an empty array to store the results

  arr.forEach(num => { // Iterate over each element of the input array using forEach
    result.push(Math.pow(num, 3)); // Raise each element to the power of 3 and push the result into the result array
  });

  return result; // Return the array containing the numbers raised to the third power
};
...
const addOne = (arr) => { // Declare a function named addOne which takes an array as input
  // Use map to create a new array with each value incremented by 1
